...
// OK
// int8_t volumes_good[] = { //测量等级 Lyrat ES8388
//   0,10,20,25,30,34,
//     38,42,46,50,53,
//     56,59,62,65,68,
//     71,74,76,78,80};
...
// OK
// int8_t volumes_good[] = { //测量等级 LyraTD MSC ZL38063
//   0,20,30,40,45,50,
//     55,60,65,69,72,
//     75,78,81,84,87,
//     90,93,96,98,100};
...
// OK
// int8_t volumes_good[] = { //测量等级 Lyrat-mini ES8311
//   0,10,20,25,30,34,
//     38,42,46,50,53,
//     56,59,62,65,68,
//     71,74,76,78,80};

int8_t * volumes = volumes_init;

volatile int set_volume = 0;
volatile int play_label = 0;
...
static void timer_cb(void *arg)
{
    ...
    if (play_label) {
        ...
        esp_audio_vol_set(player, volumes[num]);
        ets_printf("volume:%d set_volume:%d\t", num*5, volumes[num]);
        // ESP_LOGW("timer_cb", "index:%d set_volume:%d", num, volumes[num]);
    }
}
    ...
    if (evt->type == INPUT_KEY_SERVICE_ACTION_CLICK) {
        switch ((int)evt->data) {
            case INPUT_KEY_USER_ID_PLAY:
                ...
                if (play_num & 0x01) {
                    play_label = 1;
                    ESP_LOGW("input key","start");
                } else {
                    play_label = 0;
                    ESP_LOGW("input key","stop");
                }
                break;
            case INPUT_KEY_USER_ID_SET:
                ...
                set_volume = 0;
                if(set_num & 0x01){
                    volumes = volumes_good;
                    ESP_LOGW("input key","volumes_good");
                } else {
                    volumes = volumes_init;
                    ESP_LOGW("input key","volumes_init");
                }
            ...
            case INPUT_KEY_USER_ID_VOLUP:
                ...
                index = set_volume/5;
                ESP_LOGW("input key","volume:%d value:%d",set_volume,volumes[index]);
                esp_audio_vol_set(player, volumes[index]);
                break;
...
const periph_console_cmd_t cli_cmd[] = {
    /* ======================== Esp_audio ======================== */
    {
        .cmd = "pl",        .id = 1, .help = "Play music, cmd:\"play [index or url] [byte_pos]\",\n\
    ...
    { .cmd = "sv",          .id = 4,  .help = "Set volume",               .func = cli_set_vol },
    ...
    /* ======================== Playlist ======================== */
    { .cmd = "sc",        .id = 40, .help = "Scan sdcard music file, cmd: \"scan [path]\",e.g. \"scan /sdcard\"",             .func = playlist_sd_scan },
...
static void cli_setup_player(void)
{
    ...
    input_cfg.based_cfg.task_stack = 6*1024;
    ...
    esp_timer_create(&wakeup_timer_cfg, &cycle_timer);
    esp_timer_start_periodic(cycle_timer, 1000000);
...
void app_main(void)
{
    ...
    esp_log_level_set("DRV8311", ESP_LOG_DEBUG);
    esp_log_level_set("ES8388_DRIVER", ESP_LOG_DEBUG);
    esp_log_level_set("HEADPHONE", ESP_LOG_DEBUG);
    esp_log_level_set("zl38063", ESP_LOG_DEBUG);
